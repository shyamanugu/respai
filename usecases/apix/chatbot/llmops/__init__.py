"""APIX chatbot <-> LLMOps platform adapter.

Importing this package locates and executes ``platform/bootstrap.py`` (fail-open)
so the platform libraries are importable by bare name, and exposes the helpers
the chatbot wires in at its LLM choke point (``llm.client.generate_completion``).
Fail-open: if the platform tree is absent, ``PLATFORM_AVAILABLE`` is False and
the chatbot runs unchanged with no tracing/guardrails.
"""
