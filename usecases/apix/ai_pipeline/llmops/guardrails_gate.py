"""Guardrail gate for APIX ai_pipeline. Wraps the platform's
``build_guardrail("apix", env)`` with fail-open input/output checks. If the
platform is absent or a check errors, traffic is allowed (never drop a call
because observability/guardrails are unavailable).
"""
